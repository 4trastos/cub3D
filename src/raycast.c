...
void	rays(t_g *game)
{
	...
	while (++nr < WIDTH)
	{
		...
		if (rays[nr].ra > 0 && rays[nr].ra < PI)
		{
			...
			if (rays[nr].ra >= (90 * RADIAN))
				rays[nr].rx = sin(rays[nr].ra - (90 * RADIAN))  * -1 ;
			else
				rays[nr].rx = sin((90 * RADIAN) - rays[nr].ra);
		}
		// if (rays[nr].ra > PI && rays[nr].ra < (PI * 2))
		// {
...
void	key_press(mlx_key_data_t key, void *param)
{
	...
	game = param;
	dx = cos(game->p.angle) * 5.0f;
	dy = sin(game->p.angle) * 5.0f;
...
void    init_window(t_design *catridge)
{
	...
	ft_memset(game.p.player->pixels, 0, game.p.player->width * game.p.player->height * sizeof(int32_t));
